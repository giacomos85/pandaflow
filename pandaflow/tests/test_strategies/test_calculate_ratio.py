...
def test_basic_ratio(sample_df):
    ...
    result = CalculateRatioStrategy(transformation).run(sample_df)
...
def test_missing_numerator(sample_df):
    ...
    with pytest.raises(ValueError, match="Numerator column 'revenue' not found"):
        CalculateRatioStrategy(transformation).run(sample_df)
...
def test_division_by_zero():
    ...
    result = CalculateRatioStrategy(transformation).run(df)
