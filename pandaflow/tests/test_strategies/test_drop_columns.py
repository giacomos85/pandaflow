...
def test_drop_single_column(sample_df):
    transformation = {"strategy": "drop_columns", "columns": ["gender"]}
...
def test_drop_multiple_columns(sample_df):
    transformation = {"strategy": "drop_columns", "columns": ["gender", "country"]}
    result = DropColumnsStrategy(transformation).run(sample_df)
    assert result.columns.tolist() == ["name", "age"]


def test_drop_missing_column_raise(sample_df):
    transformation = {"strategy": "drop_columns", "columns": ["unknown"]}
    with pytest.raises(KeyError):
        DropColumnsStrategy(transformation).run(sample_df)
